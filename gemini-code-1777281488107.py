"""
EMM 97% Hybrid Forecasting — Streamlit Dashboard  ·  v3.0 (Updated Vis)
==========================================================
...
def render_steel_calculator(
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Mn Value (Rs/Kg Mn)", overlaying="y", side="right", showgrid=False),
    ...
    with st.expander("📋 Steel Value Table (sampled — every 4 weeks)"):
        ...
        st.dataframe(
            tbl.style.format({"Index Price Rs/Kg": "{:.2f}", "Mn Value Rs/Kg": "{:.2f}"})
               .applymap(lambda v: "color: #f0a030" if v == "Forecast" else "", subset=["Period"]),
